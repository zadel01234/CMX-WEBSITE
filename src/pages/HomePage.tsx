...
import {
  ArrowRight, Calendar, MapPin, Users, Zap, Globe, Shield,
  Briefcase, Rocket, ChevronRight, Twitter, Linkedin,
  Monitor, Building2, X, Ticket, Gift,
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { cn } from '../utils';
...
const REGISTER_LINKS = {
  virtual: 'https://events.cmxhub.com/events/details/cmx-ibadan-presents-cmx-nigeria-summit/',
  physicalPaid: 'https://flutterwave.com/pay/qg5vsnsiwnpd',
  physicalFree: 'https://luma.com/3tot21u4/',
};

// ─── Register Modal ─────────────────────────────────────────────────────────────
const RegisterModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  const [step, setStep] = useState<'attendance' | 'ticket'>('attendance');

  useEffect(() => {
    if (isOpen) setStep('attendance');
  }, [isOpen]);

  const handleVirtual = () => {
    window.open(REGISTER_LINKS.virtual, '_blank', 'noopener,noreferrer');
    onClose();
  };

  const handlePhysical = () => setStep('ticket');

  const handleTicket = (type: 'paid' | 'free') => {
    window.open(type === 'paid' ? REGISTER_LINKS.physicalPaid : REGISTER_LINKS.physicalFree, '_blank', 'noopener,noreferrer');
    onClose();
  };
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 sm:p-8"
          >
            <button
              onClick={onClose}
              aria-label="Close"
              className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center rounded-full bg-cmx-gray hover:bg-gray-200 transition-colors"
            >
              <X size={18} />
            </button>
            ...
            {step === 'attendance' ? (
              <>
                <h3 className="text-2xl sm:text-3xl font-black mb-2">How will you attend?</h3>
                <p className="text-sm sm:text-base text-gray-600 mb-6 sm:mb-8">
                  Let us know if you'll be joining us virtually or in person.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <button
                    onClick={handleVirtual}
                    className="flex flex-col items-center gap-3 p-5 sm:p-6 rounded-2xl border-2 border-cmx-gray hover:border-cmx-orange hover:bg-cmx-orange/5 transition-colors text-center"
                  >
                    <div className="w-12 h-12 rounded-full bg-cmx-orange/10 flex items-center justify-center">
                      <Monitor className="text-cmx-orange" size={24} />
                    </div>
                    <span className="font-bold text-sm sm:text-base">Virtual</span>
                    <span className="text-xs text-gray-500">Join online from anywhere</span>
                  </button>
                  <button
                    onClick={handlePhysical}
                    className="flex flex-col items-center gap-3 p-5 sm:p-6 rounded-2xl border-2 border-cmx-gray hover:border-cmx-blue hover:bg-cmx-blue/5 transition-colors text-center"
                  >
                    <div className="w-12 h-12 rounded-full bg-cmx-blue/10 flex items-center justify-center">
                      <Building2 className="text-cmx-blue" size={24} />
                    </div>
                    <span className="font-bold text-sm sm:text-base">Physical</span>
                    <span className="text-xs text-gray-500">Attend in Ibadan, Nigeria</span>
                  </button>
                </div>
              </>
            ) : (
              <>
                <button
                  ...
                  className="text-xs font-bold text-gray-400 hover:text-cmx-blue mb-3 flex items-center gap-1"
                >
                  &larr; Back
                </button>
                <h3 className="text-2xl sm:text-3xl font-black mb-2">Choose your ticket</h3>
                <p className="text-sm sm:text-base text-gray-600 mb-6 sm:mb-8">
                  Select a free or paid ticket for the physical event.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <button
                  ...
                  >
                    <div className="w-12 h-12 rounded-full bg-cmx-blue/10 flex items-center justify-center">
                      <Gift className="text-cmx-blue" size={24} />
                    </div>
                    <span className="font-bold text-sm sm:text-base">Free</span>
                    <span className="text-xs text-gray-500">General access</span>
                  </button>
                  <button
                    onClick={() => handleTicket('paid')}
                    className="flex flex-col items-center gap-3 p-5 sm:p-6 rounded-2xl border-2 border-cmx-gray hover:border-green-600 hover:bg-green-600/5 transition-colors text-center"
                  >
                    ...
                    <span className="text-xs text-gray-500">Premium access</span>
                  </button>
                </div>
              </>
            )}
      ...
      <div className="container-custom relative z-10">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-12 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <button onClick={() => setIsRegisterOpen(true)} className="btn-primary flex items-center justify-center gap-2">
                Register <ArrowRight size={16} />
              </button>
              <button className="btn-outline" onClick={() => document.getElementById('structure')?.scrollIntoView({ behavior: 'smooth' })}>
                View Schedule
